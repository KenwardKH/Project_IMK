...
export function NavbarSection() {
    ...
    const navLinks = [
        { title: 'Buat Pesanan', href: '/cashier' },
        { title: 'Konfirmasi Pesanan', href: '#' },
        { title: 'Status Pesanan', href: '#' },
        { title: 'Stok Barang', href: '#' },
    ];
    ...
    return (
        <nav className="sticky top-0 z-50 w-full bg-[#f6f6f6] py-2 shadow-sm">
            <div className="mx-auto flex h-16 max-w-screen-xl items-center justify-between px-4 md:px-6">
                ...
                {/* Desktop Navigation */}
                {navLinks.map((link, i) => {
                    const isActive = currentUrl === link.href || (link.href !== '/cashier' && currentUrl.startsWith(link.href));
                    return (
                        <Link
                            key={i}
                            href={link.href}
